...
void sig_handler(int signo, siginfo_t * sig, void * context){
    ...
    long int fp = p->uc_mcontext.regs[29], lr = p->uc_mcontext.regs[30], sp = p->uc_mcontext.sp, ret;		//获取栈回溯所需的值
    ...
    switch(signo){
        case 11:
            printf("process[%d] is going to exit because of SIGSEGV\n", cur_pid);
            exit(11);
            break;

        case 4:
            printf("process[%d] is going to exit because of SIGILL\n", cur_pid);
            exit(4);
            break;

        case 7:
            printf("process[%d] is going to exit because of SIGBUS\n", cur_pid);
            exit(7);
            break;
    }
